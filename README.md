...
recovery_target_angle_deg    로봇 중심에서 55~75% 라인 목표점을 보는 각도
...
단독 `RECOVER_LEFT/RIGHT`는 발행하지 않습니다. 번호형 복귀 회전은 화면 높이의 `55~75%` 구간에 있는 라인점들을 피팅하고, 로봇 하단 중심에서 피팅선의 `55%` 위치를 바라보는 목표각으로 판정합니다. 구간에 점이 하나뿐이면 그 점을 사용하고, 점이 없으면 기존 근거리 heading을 사용합니다. 목표각 절댓값이 `15도` 이상이고 실제 곡선 방향과 일치하지 않을 때 `RECOVER_*_TURN_LEFT/RIGHT_1~6`을 발행합니다. `RECOVER_LEFT/RIGHT` 부분은 라인이 있는 쪽이고, 실제 회전 방향은 `TURN_LEFT/RIGHT`입니다. 목표각은 `15`, `30`, `45`, `60`, `75`, `90`도의 6단계로 양자화합니다. 경계는 `15~22.5도 미만`, `22.5~37.5`, `37.5~52.5`, `52.5~67.5`, `67.5~82.5`, `82.5도 이상`이며 각 경계값은 다음 단계에 포함됩니다.
...
ros2 launch mission_control full_system.launch.py \
  ...
  recovery_target_y_min_ratio:=0.55 \
  recovery_target_y_max_ratio:=0.75
```
